...
- Обновлено: 2026-07-15 07:45 +05:00
- PRD: `PRD_Realtime_Translator_iOS_30_days_v0.1.docx`, версия 0.1 от 2026-07-13
- Состояние проекта: `READY_FOR_PARALLEL_WORK`
- Git: `origin/main` = `7d3a6af2aac4f1334c30cfa90df84a27fb02d0f5` после merge PR #19 (IOS-12); репозиторий `https://github.com/YergZakon/translator.git`
- Codex: backend owner; BE-04 merged, следующая backend-задача требует отдельной reservation/worktree
- Claude Code: IOS-12 reconnect consumer IN_PROGRESS; новые iOS-ветки `claude/ios-<task-id>-<slug>` (это исключение зафиксировано ниже в H-013 — платформенный runner ограничил работу текущей веткой issue)
...
| IOS-12 | iOS | P0 reconnect consumer и atomic translation-leg replacement | Claude Code | IN_PROGRESS | `claude/issue-18-20260715-0042` (см. H-013: платформенный runner запретил создание отдельной ветки для issue-triggered запуска) | `apps/ios/RealtimeTranslator/RealtimeTranslator/BackendClient/*`, `apps/ios/RealtimeTranslator/RealtimeTranslator/TranslationDomain/ReconnectCoordinator.swift`, `TranslationSessionStore.swift`, iOS tests | BE-04 (merged), D-006 | В процессе: см. хронологию |
...
| QA-01 | Shared/QA | Безопасный macOS-сборщик отчёта physical-iPhone reconnect acceptance | Codex | IN_REVIEW | `codex/qa-ios12-device-report` | `scripts/collect_ios_reconnect_acceptance.sh`, `scripts/lib/devicectl_device_info.js`, iOS README | IOS-12 merged, stage available | Bash syntax/help; JXA/Node parser fixture; diff/secret checks clean; реальный device run на Mac обязателен |
...
| `POST /v1/translation-sessions/{id}/legs` | Пересоздание leg при reconnect | Bearer app token | `Idempotency-Key` | Codex | IMPLEMENTED — BE-04 merged; IOS-12 consumer in progress |
...
| `SessionService.recreateLeg` | session/clientLegId/reason → fresh leg credentials | Owner/session/client leg проверяются; новый `legId` и secret; старый secret не переиспользуется | Codex | IMPLEMENTED — BE-04 in review |
...
| H-013 | Codex | Claude Code | `[AGENT_HANDOFF]` IOS-12 revision 1 получен и зарезервирован; PR #16 подтверждён смерженным (`main` = `30fa0ff42d301f424d60640e19307bfaee3e7977`, совпадает с `expected_base_head`), поэтому BE-04/H-012 обновлены на DONE/CLOSED | GitHub issue #18; этот файл | `git rev-parse HEAD` совпал с `expected_base_head` из handoff до начала правок | Автоматизированный issue-runner запрещает создание отдельной ветки `claude/ios-ios-12-reconnect`; работа выполнена на выданной ветке `claude/issue-18-20260715-0042` вместо неё — зафиксировано как известное отклонение от `required_scope`, а не тихое расширение `allowed_actions` | Claude Code возвращает `[AGENT_RESULT]` в issue #18 с PR URL, exact SHA, CI run и test count | OPEN — ожидает `[AGENT_RESULT]` |
...
|---|---|---|---|---|---|
| 2026-07-15 07:45 +05:00 | Codex | QA-01 implementation ready | Добавлен macOS helper: выбирает paired physical iPhone через Xcode `devicectl`, не сохраняет identifier, автоматически получает model/iOS/Xcode/Git/stage health и интерактивно фиксирует только PASS/FAIL, audio route и ограниченную error category; отчёт создаётся с mode 600 на Desktop; iOS README дополнен командой запуска | `bash -n`; `--help`; `node --check`; parser fixture (iPhone 15 Pro / iOS 18.5); `git diff --check`; scoped token-pattern scan; Windows не использовался для ложного device acceptance | Commit/push и draft PR; владелец запускает helper на Mac с физическим iPhone; только реальный отчёт закрывает reconnect acceptance |
| 2026-07-15 07:30 +05:00 | Codex | QA-01 start | Создан отдельный worktree от `origin/main` = `7d3a6af`; зарезервирован macOS-скрипт, который получает модель iPhone/iOS через штатный Xcode `devicectl`, Xcode/Git/stage metadata автоматически и записывает только операторские PASS/FAIL без audio/transcript/tokens | Worktree clean; актуальный merge PR #19 подтверждён до начала правок | Реализовать сборщик и parser; выполнить статические проверки; передать владельцу команду запуска на Mac; device acceptance не закрывать до реального запуска |
