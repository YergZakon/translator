...
- Обновлено: 2026-07-14 16:27 +05:00
- PRD: `PRD_Realtime_Translator_iOS_30_days_v0.1.docx`, версия 0.1 от 2026-07-13
- Состояние проекта: `READY_FOR_PARALLEL_WORK`
- Git: `origin/main` = `8e556b2` после merge IOS-05 / PR #9; репозиторий `https://github.com/YergZakon/translator.git`
...
| H-006 | Codex | Claude Code | BE-02 создаёт 1–2 short-lived translation secrets через official translation endpoint | `apps/backend/src/services/openai-secret-broker.ts`, `session-service.ts`, `POST /v1/translation-sessions` | Typecheck; mocked upstream tests; backend/iOS installation auth merged | Реальный OpenAI вызов, stage deploy и physical iPhone не выполнялись; session idempotency process-local | Выполнить stage E2E: installation → app token → config → backend secret → SDP → remote audio/transcript на physical iPhone | OPEN |
