...
- Обновлено: 2026-07-15 12:31 +05:00
- PRD: `PRD_Realtime_Translator_iOS_30_days_v0.1.docx`, версия 0.1 от 2026-07-13
- Состояние проекта: `READY_FOR_PARALLEL_WORK`
- Git: `origin/main` = `db6f2485fbf56de445fc4ce081e965ad0cb17de2` после merge PR #27 (BE-07 closeout); репозиторий `https://github.com/YergZakon/translator.git`
- Codex: backend owner; BE-08 PostgreSQL feedback API зарезервирован и IN_PROGRESS; merge/deploy отдельно
...
| BE-08 | Backend | PostgreSQL-backed owner-scoped feedback upsert с consent-gated redacted comment | Codex | IN_PROGRESS | `codex/be-08-feedback` | migration 005, accepted feedback route schemas/service/repositories/tests; без изменения OpenAPI/Swift DTO | BE-05, BE-07, API-01 | Reserve first; require PostgreSQL concurrency/restart/privacy tests, exact-head Backend CI and Claude H-018 review; merge/deploy отдельно |
...
| D-010 | 2026-07-15 | PROPOSED | BE-08 реализует принятый feedback endpoint без contract/iOS изменений: owner-scoped PostgreSQL upsert хранит один mutable record на session; rating/categories заменяются целиком; comment сохраняется только при `consentFlags.storeComment=true`, проходит server-side redaction и никогда не логируется, иначе хранится `NULL` | Codex | Claude Code H-018 | Повторная отправка безопасно обновляет запись; ownership маскируется единым 404; raw audio/transcript/secrets не принимаются и не попадают в feedback/logs |
...
| H-018 | Codex | Claude Code | BE-08 feedback producer и privacy semantics | Ветка `codex/be-08-feedback`; accepted OpenAPI feedback DTO; D-010 | PostgreSQL/in-memory upsert, concurrency/restart, ownership, validation, consent/redaction and no-log tests; exact-head CI pending | Read-only consumer/privacy review only; no iOS/backend edits, merge, deploy or secrets | Claude confirms Swift DTO compatibility, consent behavior, one-record upsert, ownership/error semantics and returns structured verdict; Codex ACK после exact-head CI | OPEN |
