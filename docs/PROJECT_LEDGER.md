...
- Обновлено: 2026-07-15 05:47 +05:00
- PRD: `PRD_Realtime_Translator_iOS_30_days_v0.1.docx`, версия 0.1 от 2026-07-13
- Состояние проекта: `READY_FOR_PARALLEL_WORK`
- Git: `origin/main` = `30fa0ff` после merge BE-04 / PR #16; репозиторий `https://github.com/YergZakon/translator.git`
...
| UX-02 | iOS | Реализация принятого визуального дизайна EasyTalk (claude.ai/design prototype) для всех экранов | Claude Code | IN_REVIEW | `claude/ios-ux-02-easytalk-design` | `TranslationUI/EasyTalkTheme.swift` (новый), рестайлинг Onboarding/Home/Live/Preflight/Result/Diagnostics | UX-01, IOS-04 | Visual-only: store/API/transport не изменены; backend/contracts diff пуст; exact-head macOS CI required |
| IOS-12 | iOS | Reconnect consumer: клиентский recreate leg через `POST /v1/translation-sessions/{id}/legs` | Claude Code | TODO | — | `LiveBackendClient` recreate call, reconnect policy в store, retry/idempotency-key handling, tests | BE-04, D-006 | Передано Codex → Claude (UX-02-FINAL handoff); старт после merge PR #13 и отдельной reservation |
