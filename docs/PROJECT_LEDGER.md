...
- Обновлено: 2026-07-14 19:39 +05:00
- PRD: `PRD_Realtime_Translator_iOS_30_days_v0.1.docx`, версия 0.1 от 2026-07-13
- Состояние проекта: `READY_FOR_PARALLEL_WORK`
- Git: `origin/main` = `204b693` после merge H-006 / PR #15; репозиторий `https://github.com/YergZakon/translator.git`
- Codex: backend owner; BE-04 зарезервирован в отдельном worktree
...
| BE-04 | Backend | Пересоздание translation leg с новым short-lived secret при reconnect | Codex | IN_REVIEW | `codex/be-04-leg-recreate` | `POST /v1/translation-sessions/{sessionId}/legs`; ownership/idempotency; backend tests | BE-02, API-01, H-006 | Local typecheck/build green; 34 tests: 33 pass + expected PostgreSQL skip; exact-head CI and Claude iOS-consumer review pending |
...
| `POST /v1/translation-sessions/{id}/legs` | Пересоздание leg при reconnect | Bearer app token | `Idempotency-Key` | Codex | IMPLEMENTED — BE-04 in review |
...
| H-012 | Codex | Claude Code | BE-04 leg recreate producer готов к iOS-consumer review | Ветка `codex/be-04-leg-recreate`; accepted OpenAPI route `POST /v1/translation-sessions/{sessionId}/legs`; D-006 | Typecheck/build green; local backend suite 34 tests: 33 pass, PostgreSQL test skipped only because `TEST_DATABASE_URL` is absent; fresh secret/legId, concurrent idempotency, conflict, ownership, expiry, 401 and sanitized 429 covered | Session registry and idempotency are process-local; PostgreSQL persistence/horizontal routing require a later hardening task; no iOS changes in BE-04 | Подтвердить Swift DTO/reconnect consumption and D-006 semantics; verdict APPROVED or CHANGES_REQUESTED; не начинать IOS-12 до merge BE-04 | OPEN |
