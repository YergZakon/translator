...
- Обновлено: 2026-07-14 19:33 +05:00
...
| BE-04 | Backend | Пересоздание translation leg с новым short-lived secret при reconnect | Codex | IN_PROGRESS | `codex/be-04-leg-recreate` | `POST /v1/translation-sessions/{sessionId}/legs`; ownership/idempotency; backend tests | BE-02, API-01, H-006 | Contract schema already accepted in API-01; require backend test/build and Claude iOS-consumer review |
...
| `POST /v1/translation-sessions/{id}/legs` | Пересоздание leg при reconnect | Bearer app token | `Idempotency-Key` | Codex | IN_PROGRESS — BE-04 |
...
| `SessionService.recreateLeg` | session/clientLegId/reason → fresh leg credentials | Owner/session/client leg проверяются; новый `legId` и secret; старый secret не переиспользуется | Codex | IN_PROGRESS — BE-04 |
