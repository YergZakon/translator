...
- Обновлено: 2026-07-15 11:08 +05:00
...
- Codex: backend owner; BE-07 is IN_REVIEW in Draft PR #26; exact-head CI and Claude H-017 pending; merge/deploy require separate authorization
...
| BE-07 | Backend | PostgreSQL-backed `POST /v1/translation-sessions/{sessionId}/complete` и немедленное освобождение parallel quota slot | Codex | IN_REVIEW | `codex/be-07-session-complete` / PR #26 | migration 004, accepted route schemas/service/repositories/tests; без изменения OpenAPI/Swift DTO | BE-05, BE-06, API-01 | Local PostgreSQL 17 suite 45/45 without skips; Draft PR exact-head Backend CI and Claude H-017 review pending; merge/deploy отдельно |
...
| `POST /v1/translation-sessions/{id}/complete` | Итоговые metadata | Bearer app token | Safe repeat | Codex | IN_REVIEW — BE-07 / PR #26 |
...
| `SessionService.complete` | result metadata → completed session | Повторный запрос безопасен; first completion wins; completion освобождает active-leg slot без refund daily reservation | Codex | IN_REVIEW — BE-07 / PR #26 |
...
| D-009 | 2026-07-15 | PROPOSED | BE-07 реализует принятый safe-repeat complete route как first-write-wins owner-scoped PostgreSQL update; completion не блокируется kill switch, запрещает дальнейший recreate и немедленно исключает session legs из parallel quota, но не возвращает уже reserved UTC-day leg-minutes | Codex | Claude Code H-017 | Stop корректно закрывает app session и освобождает concurrency без возможности обходить daily budget повторными create/complete; OpenAPI/Swift DTO unchanged |
...
| H-017 | Codex | Claude Code | BE-07 persistent session completion producer готов к bounded read-only review | Draft PR #26; implementation head `f7b144a`; migration 004; route/service/PostgreSQL and in-memory repositories/tests; D-009 | Local typecheck/build; PostgreSQL 17 suite 45/45 with 0 failures/skips; contract/iOS diff empty; exact-head GitHub CI pending | Review only: no file changes/commit/merge/deploy/secrets; confirm accepted DTO compatibility, first-write-wins repeat, owner 404, kill-switch completion, recreate rejection and quota semantics | Claude returns structured APPROVED/CHANGES_REQUESTED on final exact head; Codex verifies CI and ACK; merge/deploy remain owner-gated | OPEN |
