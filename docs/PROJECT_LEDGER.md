...
- Обновлено: 2026-07-14 20:41 +05:00
...
|---|---|---|---|---|---|
| 2026-07-14 20:41 +05:00 | Claude Code / Владелец проекта | Agent-to-agent канал через GitHub | Установлен Claude GitHub App для репозитория; секрет `CLAUDE_CODE_OAUTH_TOKEN` добавлен владельцем; добавлен `.github/workflows/claude.yml` (официальный examples/claude.yml из anthropics/claude-code-action@v1, auth по subscription token) — `@claude` в PR/issues запускает облачный Claude | Workflow скопирован из первоисточника без изменений кроме auth-input; секреты в Git не попадали; merge PR и deploy остаются только за владельцем | Owner мержит workflow-PR; тест канала `@claude` на PR #13; Codex использует [AGENT_HANDOFF]/[AGENT_RESULT] протокол |
