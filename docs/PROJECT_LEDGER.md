...
- Обновлено: 2026-07-15 09:36 +05:00
- PRD: `PRD_Realtime_Translator_iOS_30_days_v0.1.docx`, версия 0.1 от 2026-07-13
- Состояние проекта: `READY_FOR_PARALLEL_WORK`
- Git: `origin/main` = `64d7aaa55c73a38295e7de5b570b483d1d159b2c` после merge PR #21 (IOS-12 closeout); репозиторий `https://github.com/YergZakon/translator.git`
- Codex: backend owner; BE-05 зарезервирован в отдельном worktree, merge/deploy требуют отдельного разрешения владельца
...
| BE-05 | Backend | PostgreSQL persistence для translation sessions, legs и create/recreate idempotency | Codex | IN_REVIEW | `codex/be-05-session-persistence` / PR #22 | versioned migration, repository/service wiring, PostgreSQL integration tests; без изменения OpenAPI/Swift DTO | BE-03, BE-04, IOS-12, D-006 | Draft PR #22 открыт; exact-head Backend CI и read-only Claude iOS-consumer review обязательны до отдельного разрешения на merge/deploy |
...
| `SessionService.create` | validated request → app session + 1–2 legs | Одна операция/результат на idempotency key | Codex | BE-05 IN_PROGRESS — перенос ownership/idempotency из process-local storage в PostgreSQL |
| `SessionService.recreateLeg` | session/clientLegId/reason → fresh leg credentials | Owner/session/client leg проверяются; новый `legId` и secret; старый secret не переиспользуется | Codex | BE-05 IN_PROGRESS — durable session/leg ownership и recreate idempotency; IOS-12 consumer accepted |
...
| D-007 | 2026-07-15 | PROPOSED | BE-05 сохраняет D-006/OpenAPI semantics, но делает ownership sessions/legs и create/recreate idempotency durable в PostgreSQL; операции сериализуются транзакционными advisory/row locks, а replay-ответы с short-lived client secrets хранятся только в AEAD-encrypted виде с domain-separated ключом от существующего server secret | Codex | Claude Code | Рестарт и горизонтальная маршрутизация не теряют session/replay; partial/failed broker result не коммитится; crash после upstream mint может оставить только недоступный клиенту ephemeral secret и безопасно повторяется |
...
| H-015 | Codex | Claude Code | BE-05 durable PostgreSQL session/leg ownership и create/recreate idempotency готовы к read-only iOS-consumer/security review | Draft PR #22; implementation commit `95b881c`; migration 002; `SessionRepository`; PostgreSQL/in-memory implementations и integration test | Local typecheck, production build, 34/34 runnable tests green; PostgreSQL integration tests ожидают GitHub CI service; `git diff --check` clean; contracts/iOS не менялись | Replay payload с short-lived client secret хранится только AES-256-GCM ciphertext; key domain-separated от existing server secret; crash после upstream mint до commit может привести только к безопасному повторному mint; merge/deploy запрещены без отдельного разрешения | Exact-head Backend CI с обоими PostgreSQL tests без skip; Claude подтверждает D-006/DTO compatibility, D-007 persistence/encryption semantics и отсутствие нового iOS work | OPEN |
