...
- Обновлено: 2026-07-15 10:20 +05:00
...
| BE-06 | Backend | PostgreSQL QuotaService: atomic rate limiting, parallel-operation guard и daily secret-mint budget до OpenAI broker | Codex | IN_PROGRESS | `codex/be-06-quota-budget` | migration 003, quota repository/service, create/recreate integration, deterministic 429/retryAfter tests; без изменения OpenAPI/Swift DTO | BE-03, BE-05, D-007 | Draft PR, exact-head Backend CI с PostgreSQL tests и read-only Claude iOS-consumer review обязательны; merge/deploy отдельно |
