...
- Обновлено: 2026-07-15 10:25 +05:00
- PRD: `PRD_Realtime_Translator_iOS_30_days_v0.1.docx`, версия 0.1 от 2026-07-13
- Состояние проекта: `READY_FOR_PARALLEL_WORK`
- Git: `origin/main` = `2e697599069659e070aa52216f63bf9ababbd057` после merge PR #23 (BE-05 closeout); репозиторий `https://github.com/YergZakon/translator.git`
- Codex: backend owner; BE-06 зарезервирован в отдельном worktree; merge/deploy требуют отдельного разрешения владельца
...
| BE-06 | Backend | PostgreSQL QuotaService: atomic rate limiting, parallel-operation guard и daily secret-mint budget до OpenAI broker | Codex | IN_REVIEW | `codex/be-06-quota-budget` / PR #24 | migration 003, quota repository/service, create/recreate integration, deterministic 429/retryAfter tests; без изменения OpenAPI/Swift DTO | BE-03, BE-05, D-007 | Draft PR #24; exact-head Backend CI с PostgreSQL tests и read-only Claude H-016 review обязательны; merge/deploy отдельно |
...
| D-008 | 2026-07-15 | PROPOSED | BE-06 оставляет OpenAPI/iOS DTO неизменными и применяет server-side quota policy до OpenAI mint: max active legs, rolling secret mints/minute и UTC-day reserved billable leg-minutes; owner-scoped PostgreSQL advisory lock и quota rows входят в ту же транзакцию, replay bypasses charging, broker failure rolls back; отказ использует существующий `429 RATE_LIMITED` + contract-bounded `retryAfterMs` | Codex | Claude Code review pending | Атомарная cost/concurrency защита работает между инстансами; лимиты задаются runtime env и не раскрываются в client config; daily retry hint capped at contract max 1 hour |
...
| H-016 | Codex | Claude Code | BE-06 atomic PostgreSQL quota producer готов к bounded read-only consumer/security review | Draft PR #24; implementation commit `ee1612c`; migration 003; `QuotaService`; PostgreSQL/in-memory quota reservations | Local PostgreSQL 17 suite 41/41, 0 failures/skips; typecheck/build/diff/security checks green; exact-head GitHub CI pending | Claude только читает repo/PR/CI и комментирует; backend/contracts/iOS files, commits, merge, deploy и secrets запрещены; D-008 пока PROPOSED | Подтвердить checks before broker, atomic concurrency/rate/daily semantics, replay/rollback, sanitized 429 + Swift compatibility; вернуть structured `[AGENT_RESULT]` APPROVED либо bounded findings | OPEN |
