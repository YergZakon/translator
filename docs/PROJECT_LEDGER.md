...
- Обновлено: 2026-07-14 17:17 +05:00
- PRD: `PRD_Realtime_Translator_iOS_30_days_v0.1.docx`, версия 0.1 от 2026-07-13
- Состояние проекта: `READY_FOR_PARALLEL_WORK`
- Git: `origin/main` = `3e17424` после merge docs closeout / PR #11; репозиторий `https://github.com/YergZakon/translator.git`
...
| STAGE-01 | Backend/DevOps | Railway stage: PostgreSQL, production container, runtime secrets, public health и API smoke | Codex | IN_REVIEW | `codex/stage-01-railway` / PR #12 | `railway.json`; `translator-stage/stage`; `https://backend-api-stage-ee06.up.railway.app` | BE-02, BE-03, CI-02 | Deployment `85ec8cdc` SUCCESS; public health 200; live installation 201 → config 200 → session 201; runtime log secret scan clean |
...
| H-006 | Codex | Claude Code | Railway stage backend развёрнут; installation auth, config и реальный OpenAI short-lived translation secret проверены | `https://backend-api-stage-ee06.up.railway.app`; deployment `85ec8cdc`; PR #12; backend session `ts_ad4083e6cf504f59b1cbf6ed3d028076`, trace `tr_86292ebc940f4c06b2517a672fd4a15e` | Public health 200; installation 201 → config 200 with ETag → translation session 201; target `en`, model `gpt-realtime-translate`, calls host `api.openai.com`; runtime logs contain trace but no auth/app token/client secret/key pattern | Physical iPhone SDP, remote audio/transcript и route/device acceptance не выполнялись; session idempotency process-local | На физическом iPhone с Stage build выполнить SDP/WebRTC call, подтвердить remote translated audio и source/target transcript; записать device/iOS/build и новые trace/session IDs без audio/text/secrets | OPEN |
