...
- Обновлено: 2026-07-15 11:05 +05:00
...
- Codex: backend owner; BE-07 implementation locally green against PostgreSQL 17; Draft PR/CI/Claude review in progress; merge/deploy require separate authorization
...
| BE-07 | Backend | PostgreSQL-backed `POST /v1/translation-sessions/{sessionId}/complete` и немедленное освобождение parallel quota slot | Codex | IN_PROGRESS | `codex/be-07-session-complete` | migration 004, accepted route schemas/service/repositories/tests; без изменения OpenAPI/Swift DTO | BE-05, BE-06, API-01 | First completion wins; owner-bound safe replay; completed session нельзя recreate и она не учитывается active-leg quota; Draft PR/exact-head Backend CI/Claude review, merge/deploy отдельно |
